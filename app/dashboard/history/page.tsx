...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`text-[10px] px-2.5 py-1 rounded-full border font-medium capitalize inline-block ${styles[status] ?? "bg-white/10 text-gray-400 border-white/10"}`}>
      {status}
    </span>
  );
}
...
export default function HistoryPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const [{ data: bookingRows, error: bErr }, { data: donationRows, error: dErr }, { data: cardRows, error: cErr }] =
          await Promise.all([
            ...
            supabase.from("donations")
              .select("id, amount, message, created_at, status, celebrities(name, image_url)")
              .eq("user_id", uid)
              .order("created_at", { ascending: false }),
            supabase.from("fan_cards")
              .select("id, card_image_url, issued_at, status, celebrities(name, image_url)")
              .eq("user_id", uid)
        ...
        setDonations((donationRows ?? []).map((d: any) => ({
          ...
          message: d.message,
          created_at: new Date(d.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }),
        ...
        setFanCards((cardRows ?? []).map((c: any) => ({
          ...
          issued_at: new Date(c.issued_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }),
  ...
  return (
    <DashboardShell title="History" subtitle="Your bookings, donations and fan cards">
      {error && (
        <div className="mb-6 border border-red-400/20 bg-red-400/5 text-red-400 text-sm px-4 py-3 rounded-xl">{error}</div>
      )}
        ...
        {tabs.map((tab) => (
          <button key={tab.key} type="button" onClick={() => setActiveTab(tab.key)}
            className={`px-5 py-2.5 text-sm font-semibold capitalize transition border-b-2 -mb-px ${
              activeTab === tab.key ? "border-white text-white" : "border-transparent text-[var(--accent)] hover:text-white"
            }`}>
            {tab.label}
            {!loading && <span className="ml-2 text-xs opacity-60">{tab.count}</span>}
          </button>
        ))}
      </div>
      ...
      {activeTab === "bookings" && (
        <motion.div initial="hidden" animate="visible" variants={fadeUp}>
          {loading ? (
            <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-20 rounded-2xl bg-white/5 animate-pulse" />)}</div>
          ) : bookings.length === 0 ? (
            <div className="text-center py-20">
              ...
              <Link href="/celebrities" className="bg-white text-black px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-gray-200 transition">Browse Celebrities</Link>
            </div>
          ) : (
              ...
              {bookings.map((b) => (
                <motion.div key={b.id} variants={fadeUp}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[var(--foreground)] font-semibold text-sm truncate">{b.celebrity?.name}</p>
                    <p className="text-[var(--accent)] text-xs capitalize mt-0.5">{b.type.replace(/_/g, " ")} · {b.date}</p>
                  </div>
                  <div className="text-right flex-shrink-0 space-y-1">
                    <p className="text-yellow-400 font-semibold text-sm">${b.amount.toLocaleString()}</p>
                ...
                <motion.div key={d.id} variants={fadeUp}
                  ...
                  <div className="w-12 h-12 rounded-xl overflow-hidden bg-gray-800 flex-shrink-0">
                    {d.celebrity?.image_url
                      ? <img src={d.celebrity.image_url} alt={d.celebrity.name} className="w-full h-full object-cover object-top" />
                      : <div className="w-full h-full flex items-center justify-center text-xl">♥</div>}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[var(--foreground)] font-semibold text-sm truncate">{d.celebrity?.name}</p>
                    {d.message && <p className="text-[var(--accent)] text-xs mt-0.5 truncate">"{d.message}"</p>}
                    <p className="text-[var(--accent)] text-xs mt-0.5">{d.created_at}</p>
                  </div>
                  <div className="text-right flex-shrink-0 space-y-1">
                    <p className="text-yellow-400 font-semibold text-sm">${d.amount.toLocaleString()}</p>
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp}>
          {loading ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {Array.from({ length: 4 }).map((_, i) => <div key={i} className="aspect-[2/3] rounded-2xl bg-white/5 animate-pulse" />)}
            </div>
          ) : fanCards.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-4xl mb-4">🪪</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {fanCards.map((c) => (
                <motion.div key={c.id} variants={fadeUp} whileHover={{ y: -6, transition: { duration: 0.2 } }}>
                  <Link href={`/celebrities/${getCelebSlug(c.celebrity?.name ?? "")}`}>
                    <div className="relative rounded-2xl overflow-hidden aspect-[2/3] border border-[var(--border)] cursor-pointer group"
                      ...
                      {c.card_image_url
                        ? <img src={c.card_image_url} alt={c.celebrity?.name} className="absolute inset-0 w-full h-full object-cover object-top opacity-60 group-hover:opacity-80 transition" />
                        : c.celebrity?.image_url
                          ? <img src={c.celebrity.image_url} alt={c.celebrity?.name} className="absolute inset-0 w-full h-full object-cover object-top opacity-60 group-hover:opacity-80 transition" />
                          : null}
