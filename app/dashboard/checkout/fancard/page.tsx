...
export default function FanCardCheckoutPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: celeb }, { data: existing }, { data: settingsRows }, { data: profile }] = await Promise.all([
        supabase.from("celebrities").select("id, name, image_url, rating, country").eq("id", celebrityId).single(),
        user ? supabase.from("fan_cards").select("id").eq("user_id", user.id).eq("celebrity_id", celebrityId).single()
          : Promise.resolve({ data: null }),
        supabase.from("platform_settings").select("key, value"),
        user ? supabase.from("profiles").select("*").eq("id", user.id).single() : Promise.resolve({ data: null }),
      ]);
      setCelebrity(celeb);
      if (existing) setAlreadyOwned(true);
      const map: Settings = {};
  ...
  async function handleSubmit() {
    if (!celebrity || alreadyOwned) return;
...
const { error: insertErr } = await supabase.from("fan_cards").insert({
  ...
  if (success && celebrity) {
    return (
      <DashboardShell title="Fan Card Issued" subtitle="Welcome to the club">
        <div className="max-w-md mx-auto text-center py-12">
          <motion.div initial={{ scale: 0, rotate: -10 }} animate={{ scale: 1, rotate: 0 }}
            transition={{ type: "spring", duration: 0.7 }} className="mx-auto mb-8 w-44">
            <div className="relative rounded-2xl overflow-hidden aspect-[2/3] border border-yellow-400/40" style={{ backgroundColor: "#111" }}>
              ...
              <div className="absolute inset-0 p-3 flex flex-col justify-between">
                ...
                <div>
                  <p className="text-white text-sm font-bold">{celebrity.name}</p>
                  <p className="text-yellow-400 text-[9px]">★ Fan Card</p>
                </div>
              </div>
            </div>
          </motion.div>
          <h2 className="text-2xl font-bold text-[var(--foreground)] mb-2">Fan Card Issued!</h2>
          <p className="text-[var(--accent)] text-sm mb-8">
            You now own an official <span className="text-white font-semibold">{celebrity.name}</span> fan card.
          </p>
          <div className="flex gap-3 justify-center flex-wrap">
            ...
            <button onClick={() => router.push("/dashboard/history")}
              className="border border-[var(--border)] text-[var(--foreground)] px-6 py-2.5 rounded-xl text-sm font-semibold hover:border-white/40 transition">View My Cards</button>
          </div>
        </div>
      </DashboardShell>
    );
  ...
  return (
    <DashboardShell title="Checkout" subtitle="Get your fan card">
      <div className="max-w-2xl mx-auto">
        <button onClick={() => router.back()}
          className="flex items-center gap-2 text-[var(--accent)] text-sm hover:text-white transition mb-8">← Back</button>
        <h1 className="text-2xl font-bold text-[var(--foreground)] mb-8">Get Fan Card</h1>
        ...
        {loading ? (
          ...
          <motion.div initial="hidden" animate="visible" variants={fadeUp} className="space-y-6">
            ...
            {alreadyOwned && (
              <div className="border border-yellow-400/20 bg-yellow-400/5 text-yellow-400 text-sm px-4 py-3 rounded-xl">
                ⭐ You already own this fan card!{" "}
                <button onClick={() => router.push("/dashboard/history")} className="underline hover:no-underline">View it in history</button>.
              </div>
            )}

            {/* Card Preview */}
            <div className="border border-[var(--border)] rounded-2xl p-5" style={{ backgroundColor: "#0a0a0a" }}>
              <h2 className="text-[var(--foreground)] font-semibold mb-4">Fan Card Preview</h2>
              <div className="flex items-center gap-6">
                <div className="relative rounded-xl overflow-hidden flex-shrink-0 border border-yellow-400/30"
                  style={{ width: 90, aspectRatio: "2/3", backgroundColor: "#111" }}>
                  <img src={celebrity.image_url} alt={celebrity.name}
                    className="absolute inset-0 w-full h-full object-cover object-top opacity-60" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/90 to-transparent" />
                  <div className="absolute inset-0 p-2 flex flex-col justify-between">
                    <span className="text-white text-[7px] font-bold tracking-widest">CELEB</span>
                    <div>
                      <p className="text-white text-[9px] font-bold leading-tight">{celebrity.name}</p>
                      <p className="text-yellow-400 text-[7px]">★ Fan Card</p>
                    </div>
                  </div>
                </div>
                <div className="flex-1">
                  <p className="text-[var(--foreground)] font-semibold">{celebrity.name}</p>
                  <p className="text-[var(--accent)] text-xs mt-1">⭐ {celebrity.rating} rating · 📍 {celebrity.country}</p>
                  <p className="text-[var(--accent)] text-xs mt-3">Limited edition digital fan card. Verified and exclusive to CELEB platform.</p>
                </div>
              </div>
              <div className="border-t border-[var(--border)] mt-4 pt-4 flex justify-between">
                <span className="text-[var(--accent)] text-sm">Price</span>
                <span className="text-[var(--foreground)] font-bold text-lg">${FAN_CARD_PRICE}</span>
              </div>
            </div>
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-t border-[var(--border)]">
                          <div className="p-4 space-y-3">
                            {[
                              ...
                              { label: "Reference", value: `FC-${Date.now().toString().slice(-6)}` },
                            ].map((row) => (
                            ...
                            <p className="text-yellow-400 text-xs pt-2 border-t border-[var(--border)]">
                              ⚠ Send exactly ${FAN_CARD_PRICE} — fan card issued after payment verified.
                            </p>
                          </div>
                        </motion.div>
                      )}
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-t border-[var(--border)]">
                          <div className="p-4 space-y-4">
                            ...
                            <div className="flex justify-between text-xs">
                              ...
                              <span className="text-white font-semibold font-mono">${FAN_CARD_PRICE} USDT</span>
                            </div>
                            <p className="text-yellow-400 text-xs pt-2 border-t border-[var(--border)]">
                              ⚠ Send only USDT on {usdtNetwork.toUpperCase()} network.
                            </p>
                          </div>
                        </motion.div>
                      )}
            ...
            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
              onClick={handleSubmit} disabled={submitting || alreadyOwned}
              ...
              {submitting ? "Processing..." : alreadyOwned ? "Already Owned" : `Pay $${FAN_CARD_PRICE} & Get Fan Card`}
            </motion.button>
            <p className="text-[var(--accent)] text-xs text-center">🔒 Encrypted & secure. Fan card appears instantly in your history.</p>
          </motion.div>
        )}
      </div>
    </DashboardShell>
